...
def Master(arg):
    ...
    if not announceBroadcastThread.isAlive():
        ...
        while(True):
            ...
            masterAcceptConnectionThread = threading.Thread(
                target=masterAcceptConnection, args=(
            (connection, address),))
            masterAcceptConnectionThread.start()
            print ("Before terminating accept")
            if isAnnouncementOn == False:
                break
        print ("After terminating accept")
        clientIpSegmentMap = divideFile(fileLink, clientsIp)
        # for clientIp in clientsIp:
        #     clientIpSegmentMap[clientIp]={'segment': clientFileSection[clientIp]}
        distributionMsg = MSG(
